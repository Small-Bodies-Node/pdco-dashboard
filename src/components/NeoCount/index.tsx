...
export const NeoCount = ({ cadData, dateAtDataFetch }: IProps) => {
  ...
  useEffect(() => {
    ...
    const filterDates =
      (pastDaysThreshold: number) => (datumArr: (string | null)[]) => {
        ...
        return (
          0 <= daysSinceDateFromData &&
          daysSinceDateFromData <= pastDaysThreshold
        );
      };
    ...
    // Numbers calc
    const cadDataWeekAll = cadData.data
      .filter(filterDates(7))
    ...
    const cadDataMonthAll = cadData.data
      .filter(filterDates(30))
      .filter(filterDistance());
    ...
    const cadDataYearAll = cadData.data
      .filter(filterDates(365))
      .filter(filterDistance());
            ...
            <NeoCountRows
              rows={[
                { text: "All", fontWeight: "bold" },
                {
                  ...
                  fontWeight: "bold",
                  tooltip:
                    "Objects with closest approach within 42.6km of Earth's center",
                },
                {
                  text: ">50m",
                  fontWeight: "bold",
                  tooltip: "Objects larger than 50m in diameter",
                },
              ]}
              alignment="center"
            />
